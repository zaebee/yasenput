...
jQuery(function($) {
    ...
    multySearch = {
        ...
        onKeyDown: function(e, self) {
            ...
            switch (e.which) {
                case 13:
                    //если нажали Enter при открытом списке, то отправить запрос и закрыть список
                    e.preventDefault();
                    var flag = false;
                    _.each(multisearch_data.tags, function(tag) {
                        ...
                        if (tag.name.toLowerCase() == $("#multisearch-text").val().toLowerCase()) {
                            ...
                            $('#multisearch-tags a').filter(function() {
                                return $.data(this, "id") == tag.id;
                            }).end().click();
                            flag = true;
                        }
                    })
                    if (multisearch_data.places.length > 0 && !flag) {
                        console.log('Попался --->', $('#multisearch-places a').filter(function() {
                            return $.data(this, "id") == 0;
                        }));
                        $('#multisearch-places a').filter(function() {
                            return $.data(this, "id") == 0;
                        }).click();
                    }
        ...
        onClickDrop: function(me, self) {
            ...
            // users labels
                ...
                if (multisearch_result.users.indexOf(id) != -1) {
                    ...
                    console.log('LOADED',window.currentPoints.loaded);
                    //window.currentPoints.loaded = false;
                    console.log('LOADED',window.currentPoints.loaded);
                    window.currentPoints.clearing();
                    window.currentPoints.setURL().fetch();
                    window.currentCollectionPoints.setURL().fetch();
        ...
        init: function(params) {
            ...
            $(this.p.labelField).click(function(e) {
                if (e.target == this) me.showDropField();
            });
