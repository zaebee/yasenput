;(function($) {
    ...
    var methods = {
        ...
        onEnd: function(event) {
            ...
            if ($activeElement.closest('.popupwin_add-trip').length) {
                if ((start == true)&&(move == false)) {

                    var numberStep = parseInt($activeElement.data('step')) + 1,
                        $blockTrip = $activeElement.closest('.popupwin_add-trip').find('.trip-step__block:nth-child('+numberStep+')');

                    if ($blockTrip.length) {
                        $activeElement.closest('.popupwin__viewport').animate({
                          scrollTop: $blockTrip.position().top
                        });
                    }
                }
            }
