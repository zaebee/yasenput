...
class ItemsList(PointsBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ids = [trip.id for trip in search_res_trips]
        search_res_trips = TripModels.Trips.objects.filter(
            blocks__longitude__lte=ln_right).filter(
                blocks__longitude__gte=ln_left).filter(
                    blocks__latitude__lte=lt_right).filter(
                        blocks__latitude__gte=lt_left).distinct().filter(id__in=ids)

        search_res_trips_list = search_res_trips
        #search_res_points = search_res_points_list
